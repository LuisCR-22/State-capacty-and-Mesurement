########################################
# Colombia Poverty Analysis - BENFORD ONLY (NO MYERS)
########################################
# Author: Luis Castellanos Rodriguez  
# Modified: 2025-10-27
# Purpose: 
# Analyze poverty outcomes using Benford index only
# No Myers variable, no interactions between indices
...
input_file_path <- "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/03_Data/Merge/07_V01_my_Bend_CSI_NTL_pov_depto_area-V2.dta"

# Output path - BENFORD ONLY FOLDER (note: user's spelling)
output_path <- "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/04_Outputs/Excel/Col/Poverty regressions/Bendford"
...
load_and_prepare_data <- function() {
  ...
  required_vars <- c("region_number", "ano", "benford_abs_distance", 
...
########################################
# MODEL SPECIFICATIONS - VERSION A (BENFORD BASIC)
########################################

get_model_specifications_benford_basic <- function(dependent_var) {
  message("Defining model specifications for ", dependent_var, " (Benford Only - Basic)...")
  
  # Base variables - ONLY BENFORD
  benford_var <- "benford_abs_distance"
  urban_control <- "urban_area"
  
  # Model specifications with Benford only (no Myers, no interactions)
  models <- list(
    "Model 1: Benford Basic" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control),
      has_peace = FALSE,
      description = "Benford index with urban control"
    ),
    
    "Model 2: Benford + NTL" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ ntl_q2 + ntl_q3 + ntl_q4 + ntl_q5"),
      has_peace = FALSE,
      description = "Benford + NTL quintile dummies (no interactions)"
    ),
    
    "Model 3: Benford + CSI" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ csi_q2 + csi_q3 + csi_q4 + csi_q5"),
      has_peace = FALSE,
      description = "Benford + CSI quintile dummies (no interactions)"
    ),
    
    "Model 4: Benford + Peace" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ after_peace_treaty"),
      has_peace = TRUE,
      description = "Benford + Peace treatment (no interactions)"
    ),
    
    "Model 5: Benford + NTL + Peace" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ ntl_q2 + ntl_q3 + ntl_q4 + ntl_q5 +",
                      "+ after_peace_treaty"),
      has_peace = TRUE,
      description = "Benford + NTL + Peace (no interactions)"
    ),
    
    "Model 6: Benford + CSI + Peace" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ csi_q2 + csi_q3 + csi_q4 + csi_q5 +",
                      "+ after_peace_treaty"),
      has_peace = TRUE,
      description = "Benford + CSI + Peace (no interactions)"
    )
  )
...
########################################
# MODEL SPECIFICATIONS - VERSION B (BENFORD WITH INTERACTIONS)
########################################

get_model_specifications_benford_interactions <- function(dependent_var) {
  message("Defining model specifications for ", dependent_var, " (Benford Only - With Interactions)...")
  
  # Base variables - ONLY BENFORD
  benford_var <- "benford_abs_distance"
  urban_control <- "urban_area"
  
  # Model specifications with Benford and its interactions
  models <- list(
    "Model 1: Benford Basic" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control),
      has_peace = FALSE,
    ...
    "Model 2: Benford + NTL + Benford×NTL" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ ntl_q2 + ntl_q3 + ntl_q4 + ntl_q5 +",
                      benford_var, ": ntl_q2 +", benford_var, ": ntl_q3 +",
                      benford_var, ": ntl_q4 +", benford_var, ": ntl_q5"),
      has_peace = FALSE,
      description = "Benford + NTL + Benford×NTL interactions"
    ),
    
    "Model 3: Benford + CSI + Benford×CSI" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ csi_q2 + csi_q3 + csi_q4 + csi_q5 +",
                      benford_var, ": csi_q2 +", benford_var, ": csi_q3 +",
                      benford_var, ": csi_q4 +", benford_var, ": csi_q5"),
      has_peace = FALSE,
      description = "Benford + CSI + Benford×CSI interactions"
    ),
    
    "Model 4: Benford + Peace + Benford×Peace" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ after_peace_treaty +", benford_var, ": after_peace_treaty"),
      has_peace = TRUE,
      description = "Benford + Peace + Benford×Peace interaction"
    ),
    
    "Model 5: Benford + NTL + Peace + Triple" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ ntl_q2 + ntl_q3 + ntl_q4 + ntl_q5 +",
                      benford_var, ": ntl_q2 +", benford_var, ": ntl_q3 +",
                      benford_var, ": ntl_q4 +", benford_var, ": ntl_q5 +",
      ...
      has_peace = TRUE,
      description = "Benford + NTL + Peace + Triple interactions (Benford×Peace×NTL)"
    ),
    
    "Model 6: Benford + CSI + Peace + Triple" = list(
      formula = paste(dependent_var, "~", benford_var, "+", urban_control, 
                      "+ csi_q2 + csi_q3 + csi_q4 + csi_q5 +",
                      benford_var, ": csi_q2 +", benford_var, ": csi_q3 +",
                      benford_var, ": csi_q4 +", benford_var, ": csi_q5 +",
      ...
      has_peace = TRUE,
      description = "Benford + CSI + Peace + Triple interactions (Benford×Peace×CSI)"
    )
  )
...
run_analysis_for_dependent <- function(data, dependent_var, model_version, weights_available) {
  ...
  if(model_version == "benford_basic") {
    model_specs <- get_model_specifications_benford_basic(dependent_var)
    version_suffix <- "_benford_basic"
  } else {
    model_specs <- get_model_specifications_benford_interactions(dependent_var)
    version_suffix <- "_benford_interactions"
  }
...
run_poverty_analysis <- function() {
  message("Starting Colombia Poverty Analysis - BENFORD ONLY (NO MYERS)...")
  message(paste(rep("=", 80), collapse=""))
  ...
  model_versions <- c("benford_basic", "benford_interactions")
  ...
  for(dep_var in dependent_vars) {
    ...
    # Basic
    key_basic <- paste0(dep_var, "_benford_basic")
    if(key_basic %in% names(results_summary)) {
      uw_status <- ifelse(results_summary[[key_basic]]$unweighted_success, "✅", "❌")
      w_status <- ifelse(results_summary[[key_basic]]$weighted_success, "✅", "❌")
      message("  Benford Basic: UW ", uw_status, " | W ", w_status)
    }
    
    # With interactions
    key_int <- paste0(dep_var, "_benford_interactions")
    if(key_int %in% names(results_summary)) {
      uw_status <- ifelse(results_summary[[key_int]]$unweighted_success, "✅", "❌")
      w_status <- ifelse(results_summary[[key_int]]$weighted_success, "✅", "❌")
      message("  Benford Interactions: UW ", uw_status, " | W ", w_status)
    }
    message("")
  }
...
message("\n📊 POVERTY ANALYSIS WITH BENFORD ONLY COMPLETED")
...
message("     A) Basic: Benford with controls (no interactions)")
message("     B) Interactions: Benford with Benford×NTL, Benford×CSI, Benford×Peace")
message("   - NO Myers variable included")
message("   - 2 weight options: unweighted and weighted")
message("   - Total: 16 Excel output files")
message("\n📋 Output Files Location:")
message("   ", output_path)
message("\n🔧 Technical Implementation:")
message("   - Only Benford index used as main predictor")
