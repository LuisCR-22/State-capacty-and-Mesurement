...
get_nightlight_files <- function(year) {
  ...
  # Use simplified patterns based on key identifying strings
  # This approach works regardless of the exact naming convention for different years
  file_types <- list(
    average_masked = "average_masked\\.dat\\.tif",
    cf_cvg = "cf_cvg\\.dat\\.tif", 
...
#
# Each folder should contain files with these identifying strings in their names:
# - "average_masked.dat.tif" (with or without .gz compression)
# - "cf_cvg.dat.tif" (with or without .gz compression)  
# - "median_masked.dat.tif" (with or without .gz compression)
#
# The script automatically detects files regardless of the rest of the naming structure.