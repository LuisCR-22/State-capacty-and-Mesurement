...
output_path <- "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/Measurement repo/State-capacty-and-Mesurement/Outputs/Excel"
output_path_plots_base <- "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/Outputs/PNG/Myers vs. Bendford"

current_date <- format(Sys.Date(), "%Y-%m-%d")
output_file <- file.path(output_path, paste0(current_date, "-GLD_Benford_Myers_Analysis-V3", ".xlsx"))

# Create date-based subfolder for scatter plots
...
# 9. Create Sheet 2: Correlations at country-year level
...
# 10. Create Sheet 3: Correlations at country level
...
for(country in unique(results$countrycode)) {
  country_data <- results[results$countrycode == country, ]
  
  # Need at least 3 observations for correlation
  if(nrow(country_data) >= 3) {
    country_correlations <- rbind(country_correlations,
                                 data.frame(
                                   countrycode = country,
                                   corr_myers_abs_distance = cor(country_data$standardized_myers, country_data$benford_abs_distance, use = "pairwise.complete.obs"),
                                   corr_myers_chi_square = cor(country_data$standardized_myers, country_data$benford_chi_square, use = "pairwise.complete.obs"),
                                   corr_myers_mae = cor(country_data$standardized_myers, country_data$benford_mae, use = "pairwise.complete.obs"),
                                   stringsAsFactors = FALSE
                                 ))
  }
}

# 11. Create Sheet 4: Correlations for 5-year periods
...
# 12. Create Sheet 5: Explanation of columns
...
# 13. Create scatter plots for country-year data
...
if (nrow(results) >= 3) {
  ...
  for (metric in metrics) {
    ...
    # Save plot (without "scatter_" prefix)
    ggsave(
      filename = file.path(output_path_plots, paste0(metric$var, ".png")),
      plot = p_unweighted,
      width = 10,
      height = 7,
      dpi = 300
    ...
    # Save weighted plot (without "scatter_" prefix)
    ggsave(
      filename = file.path(output_path_plots, paste0(metric$var, "_weighted.png")),
      plot = p_weighted,
      width = 10,
      height = 7,
      dpi = 300
...
# 14. Create period-based scatter plots (if we have enough periods)
if (nrow(period_correlations) >= 3) {
  ...
  if (nrow(period_data) >= 3) {
    ...
    for (metric in period_metrics) {
      ...
      # Save period plot (without "period_scatter_" prefix)
      ggsave(
        filename = file.path(output_path_plots, paste0("period_", metric$var, ".png")),
        plot = p_period,
        width = 8,
        height = 6,
        dpi = 300
      ...
      # Save weighted period plot (without "period_scatter_" prefix)
      ggsave(
        filename = file.path(output_path_plots, paste0("period_", metric$var, "_weighted.png")),
...
########################################
# 16. Create improved scatter plots with country-based colors and clear labels
# Define the output path for the new plots
improved_plots_path <- "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/Measurement repo/State-capacty-and-Mesurement/Outputs/PNG/Myers vs. Bendford"

# Create directory if it doesn't exist
dir.create(improved_plots_path, showWarnings = FALSE, recursive = TRUE)

# Function to create improved scatter plot with country-based colors and clear labels
create_improved_scatter_plot <- function(data, x_var = "standardized_myers", 
                                        y_var, y_label, correlation_value = NULL,
                                        weighted = FALSE) {
  ...
  # Using colorRampPalette to create a custom palette if we have many countries
  if (n_countries <= 8) {
    # For few countries, use a predefined palette
    country_colors <- RColorBrewer::brewer.pal(max(8, n_countries), "Set1")[1:n_countries]
  } else {
    # For many countries, create a custom palette
  ...
  # Create base plot with proper aesthetics
  p <- ggplot(data, aes_string(x = x_var, y = y_var)) +
    theme_minimal() +
    theme(
      plot.background = element_rect(fill = "white", color = NA),
      ...
      legend.position = "right",  # Put legend on the right side
    ...
    labs(
      ...
      y = y_label,
      title = paste0("Relationship between Myers Index and ", y_label),
      color = "Country"  # Make sure the legend title is "Country"
    )
  ...
  # Apply the color scale with clear labels
  p <- p + scale_color_manual(values = country_colors, 
                            # Ensure labels match the country codes
                            labels = countries,
                            # Make sure legend shows all countries
                            breaks = countries)
  ...
  # Add correlation value annotation in the lower right
  if (!is.null(correlation_value) && !is.na(correlation_value)) {
    p <- p + annotate(
      "text", 
  ...
  if (weighted) {
    p <- p + labs(size = "Number of observations") +
      guides(
        # Ensure color legend has highest priority and clear labels
        color = guide_legend(order = 1, override.aes = list(size = 3), title.position = "top"),
        size = guide_legend(order = 2, title.position = "top")
      )
  } else {
    # For non-weighted plots, still ensure the color legend is clear
  ...
  # Handle case with many countries by adjusting legend layout
  if (n_countries > 15) {
    # For many countries, use a multi-column legend
...
# Create and save the improved plots
if (nrow(results) >= 3) {
  # Check if we need to load RColorBrewer
  ...
  # Create improved unweighted plots
  for (metric in metrics) {
    # Calculate correlation
    corr_val <- cor(results$standardized_myers, results[[metric$var]], use = "pairwise.complete.obs")
    
    # Create improved plot
    p_improved <- create_improved_scatter_plot(
      data = results,
      y_var = metric$var,
      y_label = metric$label,
      correlation_value = corr_val,
    ...
    # Save improved plot
    ggsave(
      filename = file.path(improved_plots_path, paste0("improved_", metric$var, ".png")),
      plot = p_improved,
      width = 10,
      height = 7,
      dpi = 300
    ...
    # Create improved weighted plot
    p_improved_weighted <- create_improved_scatter_plot(
      data = results,
      y_var = metric$var,
      y_label = paste(metric$label, "(Weighted)"),
      correlation_value = corr_val,
    ...
    # Save improved weighted plot
    ggsave(
      filename = file.path(improved_plots_path, paste0("improved_", metric$var, "_weighted.png")),
      plot = p_improved_weighted,
      width = 10,
      height = 7,
      dpi = 300
    )
  ...
  message("Improved plots created and saved to: ", improved_plots_path)
}

# Also create improved period plots if we have period data
if (exists("period_data") && nrow(period_data) >= 3) {
  # Add a 'countrycode' column to period_data for compatibility with the function
  # We'll use the period as the "country" for coloring
  period_data$countrycode <- period_data$period
  
  # Variables to plot for periods
  period_metrics <- list(
    list(var = "benford_abs_distance", label = "Absolute Distance from Benford's Law"),
    list(var = "benford_chi_square", label = "Chi-Square Statistic"),
    list(var = "benford_mae", label = "Mean Absolute Error")
  ...
  # Create improved period plots
  for (metric in period_metrics) {
    # Calculate correlation
    corr_val <- cor(period_data$standardized_myers, period_data[[metric$var]], use = "pairwise.complete.obs")
    
    # Create improved period plot
    p_improved_period <- create_improved_scatter_plot(
      data = period_data,
      y_var = metric$var,
      y_label = paste("Period Average", metric$label),
      correlation_value = corr_val,
    ...
    # Save improved period plot
    ggsave(
      filename = file.path(improved_plots_path, paste0("improved_period_", metric$var, ".png")),
      plot = p_improved_period,
      width = 8,
      height = 6,
      dpi = 300
    )
    
    # Create improved weighted period plot
    p_improved_period_weighted <- create_improved_scatter_plot(
      data = period_data,
      y_var = metric$var,
      y_label = paste("Period Average", metric$label, "(Weighted)"),
      correlation_value = corr_val,
    ...
    # Save improved weighted period plot
    ggsave(
      filename = file.path(improved_plots_path, paste0("improved_period_", metric$var, "_weighted.png")),
      plot = p_improved_period_weighted,
      width = 8,
      height = 6,
      dpi = 300
    )
  ...
  message("Improved period plots created and saved.")
}
