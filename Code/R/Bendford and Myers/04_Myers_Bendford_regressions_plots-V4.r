########################################
# Benford Wage Analysis - Enhanced Version 2.1
########################################
# Author: Luis Castellanos Rodriguez
# Modified: 2025-07-16
# Enhanced with proper Myers/Benford sample definitions and zero wage handling
...
create_scatter_plot <- function(data, x_var, y_var, y_label, version_num, 
  ...
  version_names <- c("V00" = "Raw Data (Zeros Included)", 
                     ...
                     "V02" = "Residualized (Zeros Included)", 
                     "V03" = "No Extreme + Residualized (Zeros Excluded)")
...
format_regression_table <- function(reg_results, version_name) {
  ...
  if (nrow(results_no_fe) > 0) {
    ...
    if (!is.null(stats_no_fe) && nrow(stats_no_fe) > 0) {
      ...
      # Fallback if no model stats available
      obs_row <- sprintf("%-25s", "Observations")
      for (model in models) {
        obs_row <- paste0(obs_row, sprintf("%15s", "N/A"))
      }
...
if (nrow(results) > 0) {
  ...
  for (version_name in names(all_versions)) {
    ...
    table_filename_txt <- paste0("01_Myers_Benford_Ind_", version_name, ".txt")
