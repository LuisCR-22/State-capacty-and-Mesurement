########################################
# Colombia Attacks Analysis - FOCUSED V01 DEPT-AREA WITH QUINTILE DUMMIES
########################################
# Author: Luis Castellanos Rodriguez  
# Modified: 2025-10-26
# Purpose: 
# Analyze terrorist attacks outcomes using Myers-Benford framework
# Two model versions: (A) Myers only, (B) Myers + Benford
# Two dependent variables: Attacks, Victims
...
# Input file path (merged dataset with attacks data)
input_file_path <- "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/03_Data/Merge/09_V01_my_Bn_CSI_NTL_pov_atack_dep_area.dta"

# Output path
output_path <- "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/04_Outputs/Excel/Col/Attacks regressions"
...
load_and_prepare_data <- function() {
  ...
  # *** NO RESCALING FOR COUNT DATA ***
  message("\nAttacks variables are count data - keeping as raw counts:")
  message("  Note: Attacks and Victims are non-negative integer counts")
  message("  Both urban and rural areas have same values (department totals)")
  ...
  # Check attacks data availability
  for(attack_var in c("Attacks", "Victims")) {
    if(attack_var %in% names(data)) {
      n_available <- sum(!is.na(data[[attack_var]]))
      n_zero <- sum(data[[attack_var]] == 0, na.rm = TRUE)
      n_positive <- sum(data[[attack_var]] > 0, na.rm = TRUE)
      pct_available <- round(n_available / nrow(data) * 100, 1)
      pct_zero <- round(n_zero / n_available * 100, 1)
      range_vals <- range(data[[attack_var]], na.rm = TRUE)
      mean_val <- mean(data[[attack_var]], na.rm = TRUE)
      
      message("  - ", attack_var, ": ", n_available, " obs (", pct_available, "%)")
      message("    Zero values: ", n_zero, " (", pct_zero, "%), Positive: ", n_positive)
      message("    Range: [", range_vals[1], ", ", range_vals[2], "], Mean: ", round(mean_val, 2))
    }
  }
...
run_attacks_analysis <- function() {
  message("Starting Colombia Attacks Analysis with Myers-Benford framework...")
  message(paste(rep("=", 80), collapse=""))
  ...
  # Define dependent variables (ATTACKS VARIABLES)
  dependent_vars <- c("Attacks", "Victims")
...
results <- run_attacks_analysis()

message("\n📊 ATTACKS ANALYSIS WITH MYERS-BENFORD FRAMEWORK COMPLETED")
message("🎯 Key Features:")
message("   - 2 dependent variables: Attacks, Victims (count data)")
message("   - 2 model versions: (A) Myers only, (B) Myers + Benford")
message("   - 2 weight options: unweighted and weighted")
message("   - Total: 8 Excel output files")
message("\n📋 Output Files Location:")
message("   ", output_path)
message("\n🔧 Technical Implementation:")
message("   - Count data as dependent variables (no rescaling)")
...
message("\n⚠️ Important Notes:")
message("   - Both urban and rural areas have same Attacks/Victims values (department totals)")
message("   - Many zero values expected for observations without matched attacks")
message("   - Linear regression used for count data (consider Poisson/NB for robustness)")
