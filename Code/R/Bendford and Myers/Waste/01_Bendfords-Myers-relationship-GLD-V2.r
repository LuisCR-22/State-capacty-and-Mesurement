...
# # 6. Get list of .dta files containing "ARM" for testing
# all_files <- list.files(path = input_path, 
#                         pattern = "ARM.*\\.dta$", 
#                         full.names = TRUE,
#                         recursive = TRUE) # Added recursive to find files in subfolders
# 
# if(length(all_files) == 0) {
#   stop("No ARM .dta files found in input folder")
# }

# 6. Get list of ALL .dta files in the input folder (instead of just ARM files)
all_files <- list.files(path = input_path, 
                        pattern = "\\.dta$",  # Changed from "ARM.*\\.dta$" to "\\.dta$"
                        full.names = TRUE,
                        recursive = TRUE)
...
run_all_analyses <- function(results) {
  ...
  process_and_create_outputs <- function(data, version_num, is_residualized = FALSE) {
    ...
    excel_filename <- paste0(current_date, "-Myers_Bendford_GLD-V", version_num, ".xlsx")
    ...
    message("Version ", version_num, " plots saved to ", output_path_plots)
  }
