...
TEST_MODE <- TRUE  # Change to FALSE to process all years (2008-2024)
...
extract_region_info <- function(data) {
  # Debug: show some example values
  message("Sample region_est2 values before processing:")
  sample_values <- head(unique(data$region_est2), 10)
  for(i in seq_along(sample_values)) {
    message("  ", i, ": '", sample_values[i], "'")
  }
  ...
  message("Region number extraction results:")
  unique_regions <- unique(data$region_number[!is.na(data$region_number)])
  message("  Extracted region numbers: ", paste(sort(unique_regions), collapse = ", "))
  message("  Missing region numbers: ", sum(is.na(data$region_number)), " out of ", nrow(data))
  
  # Get region mapping
  region_mapping <- create_region_mapping()
  
  # Debug: show mapping
  message("Available region mapping:")
  for(i in 1:nrow(region_mapping)) {
    message("  ", region_mapping$region_number[i], " -> ", region_mapping$region_corrected[i])
  }
  ...
  unmapped_count <- sum(is.na(data$region_corrected) & !is.na(data$region_number))
  if(unmapped_count > 0) {
    unmapped_regions <- unique(data$region_number[is.na(data$region_corrected) & !is.na(data$region_number)])
    message("  Unmapped regions found: ", paste(unmapped_regions, collapse = ", "))
  }
...
create_scatter_plot <- function(data, x_var, y_var, y_label, version_num, 
  ...
  # Create title
  # Determine if residualization actually occurred
  n_years <- length(unique(data$ano))
  residualization_possible <- n_years > 1
  
  if (residualization_possible) {
    version_names <- c("V00" = "Raw Data (Zeros Included)", 
                       "V01" = "No Extreme Myers (Zeros Excluded)", 
                       "V02" = "Fixed Effects Residualized (Zeros Included)", 
                       "V03" = "No Extreme + Fixed Effects Residualized (Zeros Excluded)")
  } else {
    version_names <- c("V00" = "Raw Data (Zeros Included)", 
                       "V01" = "No Extreme Myers (Zeros Excluded)", 
                       "V02" = "Raw Data - Single Year (Zeros Included)", 
                       "V03" = "No Extreme Myers - Single Year (Zeros Excluded)")
  }
  
  level_names <- c("reg-tot" = "Regional Level", "reg-area" = "Regional-Area Level")
  title <- paste0(version_names[paste0("V0", version_num)], " - ", level_names[level], 
                  ": Myers Index vs ", y_label_final)
  ...
  if (level == "reg-area") {
    ...
    p <- ggplot(data, aes(x = !!x_var_sym, y = !!y_var_sym, 
                          color = region_corrected, shape = factor(urban_area))) +
      geom_point(size = 3, alpha = 0.7) +
      scale_shape_manual(name = "Area", values = c("0" = 16, "1" = 17), 
                         labels = c("0" = "Rural", "1" = "Urban"))
  } else {
    # For regional level, simple points
    p <- ggplot(data, aes(x = !!x_var_sym, y = !!y_var_sym, color = region_corrected)) +
      geom_point(size = 3, alpha = 0.7)
  }
  ...
  for (level in c("reg-tot", "reg-area")) {
    ...
    for (version_name in names(all_versions)) {
      ...
      for (measure_name in names(y_vars)) {
        ...
        # Save plots
        filename <- paste0("01_Myers_Benford_Col_", version_name, level_suffix, "_", measure_name, ".png")
        
        if (measure_name == "mae") {
          # Save MAE plots in main folder
          ggsave(file.path(output_path_plots_main, filename), 
                 p, width = 12, height = 8, dpi = 300)
        } else {
          # Save other measures in checks folder
          ggsave(file.path(output_path_plots_checks, filename), 
                 p, width = 12, height = 8, dpi = 300)
        }
      }
