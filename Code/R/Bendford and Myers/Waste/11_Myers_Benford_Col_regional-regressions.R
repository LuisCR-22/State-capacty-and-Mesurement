...
# Model 3: + rural/urban controls + interactions
# Model 4: Full model with triple interaction (myers×peace_treaty×rural)
#
# TECHNICAL FEATURES:
# - Clustered SEs by department to handle spatial correlation
# - Region+year fixed effects (avoiding multicollinearity)
  ...
  run_clustered_regression <- function(formula_str, data, include_fe = FALSE) {
    tryCatch({
      ...
      if (include_fe && length(unique(data$region_number)) > 1 && length(unique(data$ano)) > 1) {
        # Use region + year FE (not region-area to avoid multicollinearity)
...
format_regression_summary <- function(coeffs_no_fe, coeffs_fe, stats_no_fe, stats_fe, version_name, level) {
  ...
  summary_text <- paste0(summary_text, "Fixed Effects: Region + Year when applicable\n\n")
...
# - Panel B: factor(region_number) + factor(ano), errores clusterizados por región
#
# VENTAJAS DE ESTE ENFOQUE:
# - Evita multicolinealidad usando FE de región (no región-área)
# - Preserva variación rural/urbana dentro de regiones
# - Permite interacciones completas en reg-area
# - Estima efectos del tratado de paz correctamente
# - Mantiene interpretabilidad económica
#