...
get_nightlight_file_info <- function(year) {
  if (year >= 2013 && year <= 2021) {
    ...
    if (year == 2013) {
      ...
      config <- "vcmslcfg"  # 2014-2021 use vcmslcfg (stray-light corrected)
    }
    satellite <- "npp"
    ...
    config <- "vcmslcfg"
    if (year == 2022) {
      satellite <- "npp-j01"  # 2022 used dual satellite naming
    } else {
      satellite <- "npp"      # 2023-2024 simplified to npp
    }
  } else {
  ...
  return(list(
    ...
    config = config, 
    satellite = satellite,
    folder_path = file.path(nightlight_base_path, version, as.character(year))
  ))
}

# Define the three file types we need for each year
get_nightlight_files <- function(year) {
  info <- get_nightlight_file_info(year)
  
  # Use flexible patterns based on key identifying strings
  # This approach is more robust for different years and naming conventions
  file_types <- list(
    average_masked = "average_masked\\.dat\\.tif(\\.gz)?$",
    cf_cvg = "cf_cvg\\.dat\\.tif(\\.gz)?$", 
    median_masked = "median_masked\\.dat\\.tif(\\.gz)?$"
  )
...
# Function to find nighttime light files for a specific year
find_nightlight_files <- function(year) {
  ...
  for (file_type in names(file_info$file_patterns)) {
    pattern <- file_info$file_patterns[[file_type]]
    matching_files <- all_files[grepl(pattern, basename(all_files))]
    
    if (length(matching_files) > 0) {
      found_files[[file_type]] <- matching_files[1]  # Take first match
      message("    FOUND ", file_type, ": ", basename(matching_files[1]))
    } else {
      message("    MISSING ", file_type, " (pattern: ", pattern, ")")
      return(NULL)  # Return NULL if any file type is missing
    }
  }
...
extract_nightlight_by_department <- function(file_path, colombia_sf, variable_name) {
  ...
  tryCatch({
    ...
    # Return dataframe with NA values
    return(data.frame(
      region_number = colombia_sf$region_number,
      region_corrected = colombia_sf$region_corrected,
      nl_value = NA,
...
message("  Summary of nighttime light variables:")
nl_vars <- c("ntl_average_masked", "ntl_cf_cvg", "ntl_median_masked")
for (var in nl_vars) {
  if (var %in% names(combined_nightlight_data)) {
    var_summary <- summary(combined_nightlight_data[[var]])
    message("    ", var, ": Min=", round(var_summary[1], 3), 
...
message("\nNighttime light variables added:")
message("  🌙 ntl_average_masked: Average radiance with background masked (nW/cm2/sr)")
message("  🌙 ntl_cf_cvg: Cloud-free coverage count (data quality indicator)")  
message("  🌙 ntl_median_masked: Median radiance with background masked (nW/cm2/sr)")
...
message("\n💡 Next steps:")
message("  1. Validate nighttime light data patterns across years")
message("  2. Examine correlations between NTL variables and economic indicators")
message("  3. Use ntl_cf_cvg to assess data quality in cloud-prone regions")
message("  4. Compare ntl_average_masked vs ntl_median_masked for robustness")
message("  5. Analyze relationships between NTL, CSI, and Myers-Benford indices")
...
message("\n🎯 Integrated dataset ready for comprehensive nighttime light analysis!")
message("=====================================")
...
########################################
# 7. CONFIGURATION FOR ADDITIONAL YEARS (2015-2024)
########################################

# NOTE: When TEST_MODE = FALSE, you'll need to ensure the following paths exist:
# 
# V21 (2014-2021):
# "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/03_Data/EOG/V21/2015"
# "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/03_Data/EOG/V21/2016"
# ... through 2021
#
# V22 (2022-2024):  
...
#
# Each folder should contain the three file types:
# - *average_masked.dat.tif.gz (or .tif)
# - *cf_cvg.dat.tif.gz (or .tif)  
# - *median_masked.dat.tif.gz (or .tif)
#
# The script will automatically detect the correct naming patterns for each year.