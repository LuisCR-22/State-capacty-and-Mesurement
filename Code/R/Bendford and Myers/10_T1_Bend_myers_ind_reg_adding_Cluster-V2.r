########################################
# Benford Wage Analysis - Enhanced Version 3.0
########################################
# Author: Luis Castellanos Rodriguez
# Modified: 2025-08-13
# V1: Raw data (NO outlier removal)
...
results <- data.frame(countrycode = character(),
                      year = integer(),
                      n_observations_myers_v1 = integer(),
                      n_observations_benford_v1 = integer(),
                      n_observations_myers_v2 = integer(),
                      n_observations_benford_v2 = integer(),
                      traditional_myers_v1 = numeric(),
                      standardized_myers_v1 = numeric(),
                      benford_abs_distance_v1 = numeric(),
                      benford_chi_square_v1 = numeric(),
                      traditional_myers_v2 = numeric(),
                      standardized_myers_v2 = numeric(),
                      benford_abs_distance_v2 = numeric(),
                      benford_chi_square_v2 = numeric(),
                      urban_share = numeric(),
                      male_share = numeric(),
...
calculate_benford_metrics <- function(wage_data) {
  ...
  if(length(first_digits) < 100) {
    return(list(abs_distance = NA, chi_square = NA, n_obs = length(first_digits)))
  }
...
# Remove wage outliers using IQR method within each country-year
remove_individual_wage_outliers <- function(wage_data) {
  # Remove zeros and negatives first
  clean_wages <- wage_data[wage_data > 0]
  
  if(length(clean_wages) < 10) {
    return(clean_wages)  # Not enough data for outlier detection
  }
...
# Remove age outliers using IQR method within each country-year
remove_individual_age_outliers <- function(age_data) {
  # Filter to valid age range first
  ages <- age_data[age_data >= 10 & age_data <= 79]
...
process_version1 <- function(results) {
  results_v1 <- results[, c("countrycode", "year", "n_observations_myers_v1", "n_observations_benford_v1",
                            "traditional_myers_v1", "standardized_myers_v1", "benford_abs_distance_v1", 
                            "benford_chi_square_v1", "urban_share", "male_share", "literacy_share", 
                            "unemployment_share")]
  
  # Rename columns for consistency
  names(results_v1) <- c("countrycode", "year", "n_observations_myers", "n_observations_benford",
                         "traditional_myers", "standardized_myers", "benford_abs_distance", 
                         "benford_chi_square", "urban_share", "male_share", "literacy_share", 
                         "unemployment_share")
...
# V2: Individual-level outlier removal
process_version2 <- function(results) {
  results_v2 <- results[, c("countrycode", "year", "n_observations_myers_v2", "n_observations_benford_v2",
                            "traditional_myers_v2", "standardized_myers_v2", "benford_abs_distance_v2", 
                            "benford_chi_square_v2", "urban_share", "male_share", "literacy_share", 
                            "unemployment_share")]
  
  # Rename columns for consistency
  names(results_v2) <- c("countrycode", "year", "n_observations_myers", "n_observations_benford",
                         "traditional_myers", "standardized_myers", "benford_abs_distance", 
                         "benford_chi_square", "urban_share", "male_share", "literacy_share", 
                         "unemployment_share")
  
  message("Version 2 processed: Individual-level outlier removal with ", nrow(results_v2), " observations")
  return(results_v2)
}
...
for(file in all_files) {
  ...
  tryCatch({
    ...
    for(yr in years) {
      ...
      # VERSION 1: NO OUTLIER REMOVAL
      # Calculate Myers Index on ALL people in age range (raw data)
      myers_result_v1 <- calculate_traditional_myers(year_data_full$age)
      n_myers_v1 <- sum(year_data_full$age >= 10 & year_data_full$age <= 79, na.rm = TRUE)
      
      # Calculate Benford metrics on raw wage data (excluding zeros)
      raw_wages <- year_data_wages$wage_no_compen[year_data_wages$wage_no_compen > 0]
      benford_v1 <- calculate_benford_metrics(raw_wages)
      
      # VERSION 2: INDIVIDUAL-LEVEL OUTLIER REMOVAL
      # Calculate Myers Index on ages with individual outlier removal
      filtered_ages_v2 <- remove_individual_age_outliers(year_data_full$age)
      myers_result_v2 <- calculate_traditional_myers(filtered_ages_v2)
      n_myers_v2 <- length(filtered_ages_v2)
      
      # Calculate Benford metrics on wages with individual outlier removal
      filtered_wages_v2 <- remove_individual_wage_outliers(year_data_wages$wage_no_compen)
      benford_v2 <- calculate_benford_metrics(filtered_wages_v2)
      ...
      new_row <- data.frame(
        countrycode = country,
        year = yr,
        n_observations_myers_v1 = n_myers_v1,
        n_observations_benford_v1 = benford_v1$n_obs,
        n_observations_myers_v2 = n_myers_v2,
        n_observations_benford_v2 = benford_v2$n_obs,
        traditional_myers_v1 = myers_result_v1$traditional,
        standardized_myers_v1 = myers_result_v1$standardized,
        benford_abs_distance_v1 = benford_v1$abs_distance,
        benford_chi_square_v1 = benford_v1$chi_square,
        traditional_myers_v2 = myers_result_v2$traditional,
        standardized_myers_v2 = myers_result_v2$standardized,
        benford_abs_distance_v2 = benford_v2$abs_distance,
        benford_chi_square_v2 = benford_v2$chi_square,
      ...
      message("  Added year ", yr, " for country ", country, 
              " (V1: Myers n=", n_myers_v1, ", Benford n=", benford_v1$n_obs, 
              "; V2: Myers n=", n_myers_v2, ", Benford n=", benford_v2$n_obs, ")")
    }
...
if (nrow(results) > 0) {
  ...
  message("Processing Version 1 (V1): Raw data - NO outlier removal")
  results_v1 <- process_version1(results)
  
  message("Processing Version 2 (V2): Individual-level outlier removal")
  ...
  write_xlsx(excel_data_v1, file.path(output_path_excel, "04_Myers_Benford_Ind_V01_NoOutliers.xlsx"))
  ...
  write_xlsx(excel_data_v2, file.path(output_path_excel, "04_Myers_Benford_Ind_V02_IndividualOutliers.xlsx"))
  ...
  message("V1 (No outlier removal): ", nrow(results_v1), " observations")
  message("V2 (Individual-level outliers removed): ", nrow(results_v2), " observations")
  message("Output files saved to: ", output_path_excel)
  message("- 04_Myers_Benford_Ind_V01_NoOutliers.xlsx")
  message("- 04_Myers_Benford_Ind_V02_IndividualOutliers.xlsx")
  message(paste(rep("=", 60), collapse=""))
  
  # Display comparison of sample sizes
  if (nrow(results) > 0) {
    message("\nSample size comparison:")
    message("Original data collection: ", nrow(results), " country-year observations")
    
    # Calculate average reduction in sample sizes
    valid_obs <- results[!is.na(results$n_observations_myers_v1) & !is.na(results$n_observations_myers_v2), ]
    if (nrow(valid_obs) > 0) {
      avg_myers_reduction <- mean((valid_obs$n_observations_myers_v1 - valid_obs$n_observations_myers_v2) / valid_obs$n_observations_myers_v1 * 100, na.rm = TRUE)
      avg_benford_reduction <- mean((valid_obs$n_observations_benford_v1 - valid_obs$n_observations_benford_v2) / valid_obs$n_observations_benford_v1 * 100, na.rm = TRUE)
      
      message("Average reduction due to individual outlier removal:")
      message("  Myers Index: ", round(avg_myers_reduction, 1), "%")
      message("  Benford Analysis: ", round(avg_benford_reduction, 1), "%")
    }
  }
