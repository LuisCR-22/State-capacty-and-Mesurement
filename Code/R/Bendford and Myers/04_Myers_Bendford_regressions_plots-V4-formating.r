...
create_enhanced_scatter_plot <- function(data, x_var, y_var, y_label, version_num, 
  ...
  # Create simple title
  #title <- paste0("Myers Index vs ", gsub(" \\(Residualized\\)", "", y_label))
...
########################################
# MAIN PROCESSING LOOP FOR ALL VERSIONS
########################################

# Define all versions to process
all_versions <- c("V00", "V01", "V02", "V03")

# Process each version
for (version_name in all_versions) {
  message("Processing version ", version_name, " for enhanced plots...")
  ...
  if (!file.exists(excel_path)) {
    message("  Excel file not found: ", excel_filename)
    next
  }
  
  # Read data
  tryCatch({
    version_data <- read_excel(excel_path, sheet = "Results")
    
    # Extract version number for processing
    version_num <- as.numeric(substr(version_name, 3, 3))
    
    # Determine if this version uses residualized variables
    is_residualized <- version_num %in% c(2, 3)
    ...
    for (measure_name in names(y_vars)) {
      ...
      # Save enhanced plots with "_2" suffix
      unweighted_filename <- paste0("01_Myers_Benford_Ind_", version_name, "_unweighted_", measure_name, "_2.png")
      weighted_filename <- paste0("01_Myers_Benford_Ind_", version_name, "_weighted_", measure_name, "_2.png")
...
message("Enhanced plotting completed! All plots now have:")
message("- Simple, concise titles")
message("- Black X-axis line, labels and title (as standard)")
