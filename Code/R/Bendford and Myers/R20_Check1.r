########################################
# Colombia Poverty Analysis - WITH MYERS×BENFORD INTERACTION
########################################
# Author: Luis Castellanos Rodriguez  
# Modified: 2025-10-27
# Purpose: 
# Analyze poverty outcomes with Myers-Benford interaction framework
# Two model versions: (A) Myers + Benford + Interaction, (B) Myers + Benford + Interaction (expanded)
...
# Output path - NEW INTERACTION FOLDER
output_path <- "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Mesurement project/04_Outputs/Excel/Col/Poverty regressions/Interaction"
...
########################################
# MODEL SPECIFICATIONS - VERSION A (MYERS + BENFORD + INTERACTION)
########################################

get_model_specifications_interaction_basic <- function(dependent_var) {
  message("Defining model specifications for ", dependent_var, " (Myers + Benford + Interaction - Basic)...")
...
########################################
# MODEL SPECIFICATIONS - VERSION B (EXPANDED WITH MORE INTERACTIONS)
########################################

get_model_specifications_interaction_expanded <- function(dependent_var) {
  message("Defining model specifications for ", dependent_var, " (Myers + Benford + Interaction - Expanded)...")
...
run_analysis_for_dependent <- function(data, dependent_var, model_version, weights_available) {
  ...
  if(model_version == "interaction_basic") {
    model_specs <- get_model_specifications_interaction_basic(dependent_var)
    version_suffix <- "_interaction_basic"
  } else {
    model_specs <- get_model_specifications_interaction_expanded(dependent_var)
...
run_poverty_analysis <- function() {
  message("Starting Colombia Poverty Analysis with Myers×Benford INTERACTION framework...")
  message(paste(rep("=", 80), collapse=""))
  ...
  for(dep_var in dependent_vars) {
    ...
    # Basic interaction
    key_basic <- paste0(dep_var, "_interaction_basic")
    ...
    key_expanded <- paste0(dep_var, "_interaction_expanded")
    if(key_expanded %in% names(results_summary)) {
      uw_status <- ifelse(results_summary[[key_expanded]]$unweighted_success, "✅", "❌")
      w_status <- ifelse(results_summary[[key_expanded]]$weighted_success, "✅", "❌")
...
message("\n📊 POVERTY ANALYSIS WITH MYERS×BENFORD INTERACTION COMPLETED")
