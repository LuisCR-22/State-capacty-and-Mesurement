...
# 4. Merge datasets by region_number and ano
# 5. Run comprehensive analysis with CSI quintile interactions
...
process_enhanced_analysis <- function() {
  ...
  for (level in levels) {
    for (version in versions) {
      ...
      # Merge datasets (now includes CSI quintile creation)
...
create_enhanced_output <- function(all_merged_data, all_regression_results) {
  ...
  for (level in c("dept", "dept_area")) {
    ...
    summary_text <- paste0("Colombia Myers-Benford Analysis - ENHANCED WITH CSI QUINTILES\n")
    ...
    excel_data[["Enhanced_Summary"]] <- data.frame(
    ...
    if (length(level_results) > 0) {
      ...
      if (paste(level, "V01", sep = "_") %in% names(all_merged_data)) {
        ...
        if (length(available_vars) > 0) {
          excel_data[["Sample_Enhanced_Data"]] <- head(sample_data[, available_vars], 30)
        }
      }
    ...
    # Write Excel file
    output_filename <- paste0("06_ENHANCED_CSI_Myers_Benford_", level, ".xlsx")
...
message("   📊 06_ENHANCED_CSI_Myers_Benford_dept.xlsx")
message("   📊 06_ENHANCED_CSI_Myers_Benford_dept_area.xlsx")

message("\n📋 ENHANCED EXCEL SHEET ORGANIZATION:")
message("   📄 Enhanced_Summary: Complete analysis overview with CSI innovations")
...
message("   👁️ Sample_Enhanced_Data: Preview with CSI and NTL quintiles")
message("   📈 CSI_Quintile_Analysis: Descriptive statistics by CSI quintiles")

message("\n💡 CSI QUINTILE INNOVATIONS:")
message("   🏛️ Historical state capacity (pre-1500) as quintiles")
message("   🔗 CSI × Myers interactions (capacity moderation)")
message("   🕊️ CSI × Peace Treaty interactions (historical capacity × reform)")
message("   🌙 CSI × NTL interactions (historical × modern development)")
message("   📊 Triple interactions: Myers × Peace × CSI quintiles")
message("   🎯 Publication-ready analysis with theoretical depth!")