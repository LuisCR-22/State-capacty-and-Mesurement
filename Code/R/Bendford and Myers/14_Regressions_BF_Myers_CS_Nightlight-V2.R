...
########################################
# PROGRESSIVE MODEL SPECIFICATIONS WITH SMART FE
########################################

# Function to define model specifications with smart FE strategy
get_progressive_model_specs <- function(level = "dept", version = "V01") {
  ...
  if (level == "dept") {
    ...
    models <- list(
      ...
      "Model 6: Triple Interaction Core" = list(
        formula = paste(benford_var, "~", myers_var, "+ after_peace_treaty + ntl_quintile_factor +",
                        myers_var, ": after_peace_treaty +", myers_var, ": ntl_quintile_factor +",
      ...
      # CATEGORY C: CSI models (Year FE only - CSI is time-invariant)
      "Model 7: CSI + Urban Controls" = list(
        formula = paste(benford_var, "~", myers_var, "+ csi_pre1500_avg_no0 + urban_share"),
        fe_strategy = "year_only"
      ),
      
      # CATEGORY D: Complex models (No FE due to insufficient variation)
      "Model 8: Full Interaction" = list(
        formula = paste(benford_var, "~", myers_var, "+ after_peace_treaty + ntl_quintile_factor + urban_share +",
                        myers_var, ": after_peace_treaty +", myers_var, ": ntl_quintile_factor +",
                        "after_peace_treaty : ntl_quintile_factor +",
    ...
    models <- list(
      ...
      "Model 5: Myers × Urban" = list(
        formula = paste(benford_var, "~", myers_var, "+ urban_area +", myers_var, ": urban_area"),
        fe_strategy = "region_year"
      ),
      
      # CATEGORY B: Peace Treaty models (Region FE only)
      "Model 2: + Peace Treaty" = list(
        formula = paste(benford_var, "~", myers_var, "+ after_peace_treaty"),
        fe_strategy = "region_only"
      ),
      ...
      # CATEGORY C: CSI models (Year FE only)
      "Model 8: CSI + Urban Controls" = list(
        formula = paste(benford_var, "~", myers_var, "+ csi_pre1500_avg_no0 + urban_area"),
        fe_strategy = "year_only"
      ),
      
      # CATEGORY D: Complex models (No FE)
      "Model 9: Complex Interaction" = list(
        formula = paste(benford_var, "~", myers_var, "+ after_peace_treaty + ntl_quintile_factor + urban_area +",
                        myers_var, ": after_peace_treaty +", myers_var, ": ntl_quintile_factor +", myers_var, ": urban_area +",
                        "after_peace_treaty : urban_area"),
        fe_strategy = "none"
      )
    )
  }
...
########################################
# ENHANCED REGRESSION ANALYSIS FUNCTION WITH SMART FE
########################################

run_progressive_regression_merged <- function(data, level = "dept", version = "V01") {
  message("Running ENHANCED progressive regression for ", level, " level, version ", version, "...")
  
  # Get model specifications with FE strategies
  ...
  # Diagnose potential FE issues
  diagnose_fe_issues(data, level)
  
  # Function to run regression with basic approach (no FE)
  run_basic_regression <- function(formula_str, data, model_name = "") {
    tryCatch({
    ...
    tryCatch({
      ...
      if (fe_strategy == "region_year") {
        ...
        if (length(unique(model_data$region_number)) > 1) {
          ...
          message("      ✅ Added Region FE only (avoiding collinearity)")
        } else {
        ...
        if (length(unique(model_data$ano)) > 1) {
          ...
          message("      ✅ Added Year FE only (CSI time-invariant)")
        } else {
      ...
      message("      ✅ Success: ", nrow(results_df), " coefficients")
      return(list(coefficients = results_df, model_stats = model_stats))
  ...
  for (i in seq_along(model_specs)) {
    ...
    result <- run_basic_regression(model_spec$formula, data, model_name)
...
# Diagnostic function for FE issues
diagnose_fe_issues <- function(data, level = "dept") {
  message("  🔍 Diagnosing potential FE issues...")
  ...
  # Issue warnings
  if (sum(time_variation$csi_var > 0, na.rm = TRUE) == 0) {
    message("    ⚠️ CSI has no time variation → Will use Year FE only for CSI models")
  }
  if (sum(peace_variation$peace_var > 1) < nrow(peace_variation) * 0.5) {
    message("    ⚠️ Limited peace treaty variation → Will use Region FE only for peace models")
  }
}
