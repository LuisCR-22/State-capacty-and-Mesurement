...
for(file in all_files) {
  ...
  tryCatch({
    ...
    for(yr in years) {
      year_data_full <- dat[year == yr]  # Keep full data for control variables
      ...
      clean_wages <- remove_wage_outliers(year_data_wages$wage_no_compen)
      if(length(clean_wages) < 100) {
        message("  Skipping year ", yr, " due to insufficient data after outlier removal")
        next
      }
      
      # Calculate Myers Index on people with wage data (for consistency)
      myers_result <- calculate_traditional_myers(year_data_wages$age)
